import { defaultStudioTenant, neutralTestTenant, getTenantConfig, TenantConfig } from '../config/tenantConfig';
...
async function runTenantWiringTests() {
  ...
  // 1. Default Master Tenant Payment Values & Telegram Output from Tenant Config
  const defaultConfig = getTenantConfig('proj-aj-studio-01');
  assert(defaultConfig.displayName === 'AJ AI Studio', 'Default tenant displayName mismatch');
  assert(defaultConfig.invoicePrefix === 'INV-AJ', 'Default tenant invoicePrefix mismatch');
  assert(defaultConfig.receiptPrefix === 'RCP-AJ', 'Default tenant receiptPrefix mismatch');

  const ajKpayBrand = getPaymentBrand('KBZPay', defaultConfig);
  assert(ajKpayBrand.defaultBadge === 'AJ KPay', `AJ KPay badge mismatch: ${ajKpayBrand.defaultBadge}`);

  const ajGateways = getGatewayConfigs(defaultConfig);
  assert(ajGateways.KBZPay.accountName.includes('AJ AI STUDIO'), 'AJ KBZPay account name mismatch');

  const ajTgMsg = generateTelegramConfirmationMessage(sampleBookingState, defaultConfig);
  assert(ajTgMsg.includes('AJ AI STUDIO'), 'AJ Telegram message title mismatch');
  assert(ajTgMsg.includes('@AJ_AI_Studio_Bot'), 'AJ Telegram bot handle mismatch');
  assert(ajTgMsg.includes('No. 42 Strand Road'), 'AJ Telegram address mismatch');
  console.log('✅ Test 1: AJ Master tenant payment and Telegram output use default studio tenant configuration.');
  ...
  assert(!luminaPayloadStr.includes('AJ AI Studio'), 'AJ leak detected in Lumina Telegram payload');
  assert(!luminaPayloadStr.includes('ajaistudio'), 'AJ domain leak detected in Lumina Telegram payload');
  console.log('✅ Test 2: Second neutral test tenant output contains no master tenant identity.');
  ...
  const tenantKeys = Object.keys(defaultConfig);
  ...
  const webhookResult = await sendTelegramBookingWebhook(sampleBookingState, undefined, defaultConfig);
  ...
  // 6. Second Tenant Pass Contains No Master Tenant Identity or Domain
  assert(!luminaIcs.includes('AJ AI Studio'), 'Master tenant leak detected in Lumina pass calendar export');
  assert(!luminaIcs.includes('ajaistudio'), 'Master domain leak detected in Lumina pass calendar export');
  console.log('✅ Test 6: Second tenant pass contains no master tenant name, address, URL, prefix, or domain.');
  ...
  // 8. Default AJ AI Studio Archive Compatibility Remains Available
  const ajArchiveManifestTitle = defaultConfig.archiveManifestTitle || `${defaultConfig.displayName} - Master Archive Manifest`;
  const ajArchivePrefix = defaultConfig.archiveExportFilenamePrefix || 'AJ';
  assert(ajArchiveManifestTitle === 'AJ AI Studio - Master Archive Manifest', 'AJ archive manifest title mismatch');
  assert(ajArchivePrefix === 'AJ', 'AJ archive export prefix mismatch');
  assert(defaultConfig.archiveAssetFilenamePrefix === 'AJ_RAW', 'AJ RAW asset prefix mismatch');
  console.log('✅ Test 8: Native AJ Studio archive compatibility verified.');
  ...
  const ajReceipt = generateDigitalReceiptJson(sampleBookingState, defaultConfig);
  assert(ajReceipt.financialLedger.depositSettled === false, 'Deposit settled must remain false for manual verification');
  assert(
    ajReceipt.clientIdentity.verificationStatus === 'SLIP_OCR_EXTRACTED_PENDING_STUDIO_LEDGER',
    'Verification status must accurately state pending studio ledger review'
  );
  assert(
    ajReceipt.termsAndCertification.certificationStatement.includes('Final booking confirmation requires studio ledger review'),
  ...
  const digestString = ajReceipt.termsAndCertification.digitalSignatureDigest;
