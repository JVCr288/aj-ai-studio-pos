...
AJ AI Studio Platform Root
  ├── Platform Container & Shell Identity (AJ AI Studio)
  ├── Multi-Tenant Configuration Layer
  │     └── Active Pilot Tenant: AKK Photo Studio
...
1. **AJ AI Studio Platform Ownership**: AJ AI Studio is the hero brand, platform identity, and architectural owner.
2. **Tenant Parameterization**: AKK Photo Studio is the first pilot tenant. Tenant-owned configuration (studio display name, address, payment accounts, invoice headers) is parameterized and read from tenant configuration.
